...
fn validate_geometry(root: &Path, geometry: &ModelGeometry) -> Result<()> {
    ...
    if geometry.compression_ratios.len() < geometry.layers as usize {
        return invalid(
            root,
            "config `compress_ratios` must cover every hidden layer",
        );
    }
    Ok(())
}
...
fn validate_tensor(
    ...
    if !claimed.insert(name.to_owned()) {
        return invalid(
            root,
            format!("tensor `{name}` was classified more than once"),
        );
    }
    Ok(tensor.clone())
}
...
mod tests {
    ...
    fn fixture() -> SourceInventory {
        ...
        for (role, shape) in [
            ...
            tensors.insert(
                format!("layers.0.ffn.experts.0.{role}.weight"),
                tensor("I8", &[shape[0], shape[1].div_ceil(2)]),
            );
            tensors.insert(
                format!("layers.0.ffn.experts.0.{role}.scale"),
                tensor("F8_E8M0", &[shape[0], shape[1].div_ceil(32)]),
            );
        }
    ...
    fn rejects_unclassified_tensor() {
        ...
        source
            .tensors
            .insert("mystery.weight".into(), tensor("BF16", &[1]));
        assert!(DeepSeekV4Frontend::build(&source).is_err());
        fs::remove_dir_all(source.root).unwrap();
    }
    ...
    fn accepts_optional_mtp_tensor_as_classified_draft_record() {
        ...
        source
            .tensors
            .insert("mtp.0.main_proj.weight".into(), tensor("BF16", &[1]));
        let model = DeepSeekV4Frontend::build(&source).unwrap();
        assert!(
            model
                .global_tensors
                .contains_key("draft.mtp.0.main_proj.weight")
        );
        fs::remove_dir_all(source.root).unwrap();
    }
}
