...
fn apple8_exact_mxfp4_repack_matches_frozen_tile_order() {
    ...
    for index in 0..3 {
        ...
        assert_eq!(u32::from_le_bytes(lowered[desc + 36..desc + 40].try_into().unwrap()), 32);
        assert_eq!(u64::from_le_bytes(lowered[desc + 72..desc + 80].try_into().unwrap()), 0);
        ...
        assert!(lowered[matrix_offset + 137..matrix_offset + 136 + 16].iter().all(|byte| *byte == 0));
    }
...
fn apple8_compiler_quantized_mxfp4_emits_production_layout() {
    ...
    for index in 0..3 {
        let desc = 64 + index * 128;
        assert_eq!(u16::from_le_bytes(lowered[desc + 4..desc + 6].try_into().unwrap()), 0x20);
        assert_eq!(u16::from_le_bytes(lowered[desc + 6..desc + 8].try_into().unwrap()), 4);
        assert_eq!(u16::from_le_bytes(lowered[desc + 12..desc + 14].try_into().unwrap()), 0x0103);
        assert_eq!(u64::from_le_bytes(lowered[desc + 56..desc + 64].try_into().unwrap()), 136);
    }
    std::fs::remove_file(path).unwrap();
}
