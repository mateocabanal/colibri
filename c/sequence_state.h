...
static inline int coli_sequence_state_info_same(
        const ColiSequenceStateInfo *a, const ColiSequenceStateInfo *b) {
    return a && b &&
        a->state_abi == b->state_abi &&
 ...
 * ids must be unique and aggregate snapshot size must be overflow-safe. The
 * metadata-only and descriptor-bearing describe calls must agree exactly: a
 * changing boundary is not safe to snapshot. */
static inline int coli_sequence_snapshot_layout(
    ...
    if (!adapter || !adapter->ctx || !adapter->ops || !adapter->ops->describe ||
        !layout)
        return -1;
    ColiSequenceStateInfo first;
    memset(&first, 0, sizeof(first));
    if (adapter->ops->describe(adapter->ctx, absolute_position, &first, NULL, 0) != 0 ||
        !first.state_abi || first.absolute_position != absolute_position ||
        !first.segment_count || first.segment_count > layout->segment_capacity ||
        !layout->segments)
        return -1;
    ...
    if (adapter->ops->describe(adapter->ctx, absolute_position, &info,
        ...
        !coli_sequence_state_info_same(&first, &info) ||
...
static inline int coli_sequence_snapshot_read_all(
    ...
    if (!adapter || !adapter->ops || !adapter->ops->read_segment || !layout || !dst)
        return -1;
    uint64_t required = coli_sequence_snapshot_bytes(layout);
    if (required == UINT64_MAX || required > SIZE_MAX || required > dst_bytes)
        return -1;
    uint64_t total = 0;
    for (size_t i = 0; i < layout->info.segment_count; ++i) {
        uint64_t next;
        if (coli_sequence_u64_add(total, layout->segments[i].snapshot_bytes, &next) != 0 ||
            next > required || layout->segments[i].snapshot_bytes > SIZE_MAX)
            return -1;
    ...
    return total == required ? 0 : -1;
}

static inline void coli_sequence_restore_fail_empty(
        const ColiSequenceStateAdapter *adapter) {
    if (adapter && adapter->ops && adapter->ops->reset)
        (void)adapter->ops->reset(adapter->ctx);
}
...
static inline int coli_sequence_snapshot_restore_all(
    ...
    /* Preflight the complete framing before mutating live state. A truncated or
     * oversized snapshot is never allowed to reset or partially overwrite it. */
    uint64_t required = coli_sequence_snapshot_bytes(layout);
    if (required == UINT64_MAX || required > SIZE_MAX || src_bytes != (size_t)required)
        return -1;
    ...
    for (size_t i = 0; i < layout->info.segment_count; ++i) {
        ...
        if (coli_sequence_u64_add(total, layout->segments[i].snapshot_bytes, &next) != 0 ||
            next > required || layout->segments[i].snapshot_bytes > SIZE_MAX) {
            coli_sequence_restore_fail_empty(adapter);
            return -1;
        }
