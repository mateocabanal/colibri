...
int main(void) {
    ...
    CHECK(apple8 != NULL, "Apple8 fixture allocation failed");
    if (!apple8) return 1;

    char path[] = "/tmp/colibri-apple8-metalio-XXXXXX";
    int fd = mkstemp(path);
    CHECK(fd >= 0, "mkstemp failed");
    if (fd < 0) { free(apple8); return 1; }
    ...
    int file = metalio_file_add(path);
    CHECK(file >= 0, "metalio_file_add failed");
    int slot = metalio_slot_alloc(apple8_bytes);
    CHECK(slot >= 0, "metalio_slot_alloc failed");
    if (file < 0 || slot < 0) goto done;
    ...
    float x[(size_t)S * I];
    float ref[(size_t)S * O];
    float got[(size_t)S * O];
