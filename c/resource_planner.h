...
/* Shared bridge from runtime observations into planner values.
 *
 * reuse_weight is a dimensionless expected-use weight over a caller-chosen
 * planning horizon. It may be a literal expected reuse count, fixed-point
 * probability mass, decayed activation hotness, etc. The only requirement is
 * that candidates compared in one selection use the same scale/horizon.
 *
 * This lets dense tensors, routed experts, prefix state and future optional
 * resources enter the same planner without the planner knowing model names or
 * how each runtime derived its reuse estimate. bytes_per_miss may differ from
 * resident_bytes (compressed storage vs prepared resident layout), and exposed
 * time is kept separately so async I/O can reduce time benefit without lying
 * about bytes avoided. */
typedef struct {
    ColiResourceKind kind;
    uint32_t id;
...
static inline uint64_t coli_resource_saturating_mul(uint64_t a, uint64_t b) {
    return a && b > UINT64_MAX / a ? UINT64_MAX : a * b;
}

/* Convert one model/backend-neutral benefit estimate into the exact candidate
 * format consumed by the allocator. Zero reuse is valid and simply produces a
 * zero-value candidate that selection will skip. */
static inline int coli_resource_candidate_from_benefit(
        const ColiResourceBenefitEstimate *estimate,
        ColiResourceCandidate *candidate) {
    if (!estimate || !candidate || estimate->kind < COLI_RESOURCE_DENSE_TENSOR ||
        estimate->kind > COLI_RESOURCE_IO_STAGING || !estimate->resident_bytes)
        return -1;
    candidate->kind = estimate->kind;
    candidate->id = estimate->id;
    candidate->resident_bytes = estimate->resident_bytes;
    candidate->expected_bytes_avoided = coli_resource_saturating_mul(
        estimate->reuse_weight, estimate->bytes_per_miss);
    candidate->expected_exposed_ns_avoided = coli_resource_saturating_mul(
        estimate->reuse_weight, estimate->exposed_ns_per_miss);
    return 0;
}
...
/* Atomically from the caller's perspective: select only from this tier's free
 * optional envelope, then commit exactly the selected bytes. Selection itself
 * allocates nothing and mutates no plan state, so a validation/commit failure
 * leaves the tier unchanged and clears the tentative selection. */
static inline int coli_resource_plan_select_optional(
        ColiResourcePlan *plan, ColiResourceTier tier,
        const ColiResourceCandidate *candidates, size_t count,
        ColiResourceValueMode mode, unsigned char *selected,
        ColiResourceSelection *out) {
    if (selected && count) memset(selected, 0, count);
    if (out) memset(out, 0, sizeof(*out));
    if (!plan || tier < 0 || tier >= COLI_RESOURCE_TIER_COUNT || !out ||
        (!selected && count))
        return -1;

    ColiResourceSelection selection;
    uint64_t budget = plan->tier[tier].free_bytes;
    if (coli_resource_select(candidates, count, budget, mode,
                             selected, &selection) != 0) {
        if (selected && count) memset(selected, 0, count);
        return -1;
    }
    if (coli_resource_plan_commit_optional(
            plan, tier, selection.selected_resident_bytes) != 0) {
        if (selected && count) memset(selected, 0, count);
        return -1;
    }
    *out = selection;
