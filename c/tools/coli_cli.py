...
_BANNER_MODELS = (
    ...
    ("kimi",        "Kimi K3",           "2.8T"),
    ("olmoe",       "OLMoE",             "7B"),
    ("qwen3_7_moe", "Qwen3.7",           "TBD"),
    ("qwen3_6_moe", "Qwen3.6",           "35B-A3B"),
    ("qwen3_5_moe", "Qwen3.5",           "397B-A17B"),
    ("qwen3_moe",   "Qwen3 MoE",         "235B-A22B"),
    ("glm",         "GLM-5.2",           "744B"),
)
...
def engine_for(model):
    ...
    name = {"inkling": "inkling", "kimi": "kimi_k3", "olmoe": "olmoe",
            "deepseek_v4": "deepseek_v4", "qwen3_moe": "qwen_moe"}[arch]
...
def need_model(model, engine=None):
    ...
    if not os.path.exists(engine):
        ...
        target = ("kimi_k3" if arch == "kimi" else "inkling" if arch == "inkling" else
                  ...
                  "deepseek-v4" if arch == "deepseek_v4" else
                  "qwen_moe" if arch == "qwen3_moe" else "colibri")
...
def env_for_engine(a, arch):
    ...
    if arch in ("deepseek_v4", "kimi", "qwen3_moe"):
        if a.ram: env["RAM_GB"] = str(a.ram)
    if arch == "qwen3_moe":
        env["CHAT"] = "1"
        env["MAX_NEW"] = str(ngen_for(a))
        # Intentional conservative policy: RAM_GB/2 is a count of resident
        # experts per layer (not GB), leaving room for dense weights and KV.
        # An explicit CACHE wins; without either, qwen_moe defaults to topk.
        if a.ram: env.setdefault("CACHE", str(max(1, min(4096, a.ram // 2))))
    if arch in ("deepseek_v4", "qwen3_moe"):
...
def cmd_build(a):
    ...
    target={"glm":"colibri","inkling":"inkling","kimi":"kimi_k3","olmoe":"olmoe",
            "deepseek_v4":"deepseek-v4","qwen3_moe":"qwen_moe"}[arch]
    # The V4 runtime has an Apple Metal backend and the package executor registers
    # its streamed slots with it. Keep that path on by default for the public
    # launcher too; an explicit METAL=0 remains a useful CPU-only diagnostic.
    env=os.environ.copy()
    if arch=="deepseek_v4" and sys.platform=="darwin":
        env.setdefault("METAL","1")
    sys.exit(subprocess.call(["make","-C",HERE,target],env=env))
...
def cmd_run(a):
    ...
    if arch=="qwen3_moe":
        # In direct mode qwen_moe reads SNAP from argv[1] and an optional
        # explicit cap from argv[2]. Keep argv[2] absent when --cap is absent
        # so its native CACHE -> RAM_GB -> topk precedence remains observable.
        cmd=[engine,os.path.abspath(a.model)]
        if a.cap is not None: cmd.append(str(a.cap))
        result=subprocess.run(
            cmd,input=prompt+"\n",text=True,env=env_for_engine(a,arch),check=False)
        sys.exit(result.returncode)
...
def cmd_chat(a):
    ...
    if arch!="glm":
        ...
        model_id=("kimi-k3-colibri" if arch=="kimi" else
                  ...
                  "qwen3-moe-colibri" if arch=="qwen3_moe" else "deepseek-v4-colibri")
        ...
        # missing flag per model arch. Qwen preserves CACHE -> RAM_GB -> topk;
        # the other non-glm engines retain legacy 8. Explicit values, including
        # 0, pass verbatim (cap_for_arch, #379).
...
def main():
    # argparse applies a subparser's inherited defaults after options parsed by
    ...
    # measures "fast", cached in <model>/.coli_ssd). Qwen uses CACHE, then a
    # RAM-derived size, then topk; other non-glm arches retain legacy 8
    # (openai_server.cap_for_arch). An explicit --cap N -- 0 included -- reaches
    # the engine verbatim; see colibri.c's coli_resolve_cap().
    ...
    a=ap.parse_args()
    if leading_model is not None:
        a.model=leading_model
